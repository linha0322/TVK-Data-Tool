...
# pyinstaller --onefile --icon=SG.ico --clean TVK Data Tool_v1.2py
...
try:
    ...
    pass
f= open(file,"w+") # .txt file for the output
...
def Temp_Ver_45(xl_45,t,n):
    for i in range(n,xl_45.nrows):
        if (xl_45.cell_value(i, 1)<46 and xl_45.cell_value(i,1)>44):
...
def TNU_60(xl_TNU,t_TNU,n): # even the variable is 60 but it means for 37
    trigger = 0
    for i in range(n,xl_TNU.nrows):
        if i+40/2 > xl_TNU.nrows:
            trigger = 1
            ...
            if xl_TNU.cell_value(i, 1)<38 and 36<xl_TNU.cell_value(i, 1):
                if abs(xl_TNU.cell_value(i, 1)-xl_TNU.cell_value(i+1, 1))<0.5:
...
def TNU_function(file_path_TNU):
    ...
    time_delay_TNU = 30
    t_TNU = int(time_delay_TNU/2)+1 # this is selecting 10 sec before next cycle 
    # TNU data
    ...
    while trigger==0:
        ...
        if temp_Ave_60<38 and temp_Ave_60>36:
    ...
    mylist = ["94\N{DEGREE SIGN}C(R)","94\N{DEGREE SIGN}C(L)","37\N{DEGREE SIGN}C(R)","37\N{DEGREE SIGN}C(L)"]
